...
function waMsg(orgName, count, orgSlug) {
  ...
  if (count >= TARGET_2) {
    return `חבר׳ה, פתחנו בקשה לצרף את ${orgName} לבום ביי. זה יכול לתת לנו מחירי יבואן על Apple, הנחות קבועות בסופר, אופנה, חופשות והטבות נוספות, בלי תוספת תקציב לארגון. כבר עברנו את ה-20, וכל הצטרפות נוספת מחזקת את הפנייה. הצטרפו כאן: ${link}`;
  }
  if (count >= TARGET_1) {
    const remaining = TARGET_2 - count;
    return `חבר׳ה, פתחנו בקשה לצרף את ${orgName} לבום ביי. כבר ${count} עובדים הצטרפו. חסרים לנו עוד ${remaining} כדי להגיע ל-20. הצטרפו כאן: ${link}`;
  }
  const remaining = TARGET_1 - count;
  return `חבר׳ה, פתחנו בקשה לצרף את ${orgName} לבום ביי. כבר ${count} עובדים הצטרפו. חסרים לנו עוד ${remaining} כדי להגיע ל-10. הצטרפו כאן: ${link}`;
}
...
המהלך מאפשר לעובדים לקבל מחירי סיטונאות על מוצרי פרימיום, הנחות קבועות בסופר ובפארם, מתנות חג גמישות והטבות נוספות, והכול ב-0 שקל תוספת תקציב לארגון.
...
export default function OrgPage() {
  ...
  // Share is visible only after survey is done (this session or previous) OR
  // for returning already-joined users who are not in a "just joined" flow
  const showShareTools = surveyDone || (alreadyJoined && !justJoined);
