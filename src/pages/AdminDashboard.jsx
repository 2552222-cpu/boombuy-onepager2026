...
function calcLeadScore(req) {
  ...
  if (size.includes("מעל 250")) score += 4;
  else if (size.includes("50–250")) score += 2;
  else if (size.includes("עד 50")) score += 1;

  // תקציב חג
  const budget = req.holidayBudget || "";
  if (budget.includes("מעל 500")) score += 3;
  else if (budget.includes("200–500")) score += 2;
  ...
  // מספר עובדים
  score += Math.min(req.currentCount || 1, 5);
  ...
  return (
    ...
    >
      ...
      >
        ...
        <h2 style={{ fontSize: "22px", fontWeight: 900, marginBottom: "20px", letterSpacing: "-0.02em" }}>
          {req.orgName}
        </h2>
        ...
        {req.initiatorPhone && (
          <a
            href={`https://wa.me/972${req.initiatorPhone.replace(/^0/, "").replace(/-/g, "")}`}
            target="_blank"
...
export default function AdminDashboard() {
  ...
  return (
    <div
      style={{ background: "#F5F5F7", minHeight: "100vh", padding: "32px 16px", fontFamily: "var(--font-heebo)" }}
      dir="rtl"
    >
      <div className="max-w-screen-xl mx-auto">
        ...
        <div style={{ marginBottom: "24px", display: "flex", justifyContent: "space-between", alignItems: "flex-end", flexWrap: "wrap", gap: "12px" }}>
          <div>
            <h1 style={{ fontSize: "26px", fontWeight: 900, letterSpacing: "-0.02em", color: "#1D1D1F", marginBottom: "2px" }}>
              BoomBuy — ניהול לידים
            </h1>
          ...
          <div style={{ display: "flex", gap: "8px", alignItems: "center" }}>
            ...
            {[["score", "Lead Score"], ["count", "עובדים"], ["date", "תאריך"]].map(([val, label]) => (
              <button
                key={val}
                ...
                style={{
                  ...
                  borderRadius: "8px",
                  border: "none",
                  background: sortBy === val ? "#0066CC" : "#fff",
                  color: sortBy === val ? "#fff" : "#1D1D1F",
                  fontWeight: 600,
                  fontSize: "12px",
                  cursor: "pointer",
                  fontFamily: "var(--font-heebo)",
            ...
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              <thead>
                <tr style={{ background: "#F5F5F7", borderBottom: "1px solid rgba(0,0,0,0.06)" }}>
                  {["ארגון", "גודל", "תקציב חג", "פעילויות", "עובדים", "WA", "מכתב", "Lead Score", "סטטוס", "תאריך", "פעולה"].map((h) => (
                    <th key={h} style={{ padding: "11px 14px", textAlign: "right", fontSize: "11px", fontWeight: 600, color: "#86868B", whiteSpace: "nowrap" }}>
                      {h}
                    </th>
                  ))}
                </tr>
              ...
              <tbody>
                {sorted.map((req, i) => {
                  ...
                  return (
                    ...
                    >
                      <td style={{ padding: "12px 14px", fontWeight: 700, fontSize: "13px", color: "#1D1D1F", whiteSpace: "nowrap" }}>
                        {req.orgName}
                        {req.initiatorName && (
                          <div style={{ fontSize: "11px", color: "#86868B", fontWeight: 400 }}>
                            {req.initiatorName}{req.initiatorPhone && ` · ${req.initiatorPhone}`}
                          </div>
                        )}
                      </td>
                      <td style={{ padding: "12px 14px", fontSize: "12px", color: "#1D1D1F", whiteSpace: "nowrap" }}>{req.orgSize || "—"}</td>
                      <td style={{ padding: "12px 14px", fontSize: "12px", color: "#1D1D1F", whiteSpace: "nowrap" }}>{req.holidayBudget || "—"}</td>
                      <td style={{ padding: "12px 14px", fontSize: "12px", color: "#1D1D1F", maxWidth: "130px" }}>
                        {req.activities?.length ? req.activities.join(", ") : <span style={{ color: "#C7C7CC" }}>—</span>}
                      </td>
                      <td style={{ padding: "12px 14px", textAlign: "center", fontWeight: 800, fontSize: "15px", color: "#0066CC" }}>
                        {req.currentCount || 1}
                      </td>
                      <td style={{ padding: "12px 14px", textAlign: "center", fontSize: "14px" }}>
                        {req.whatsappCopied ? <span style={{ color: "#34C759" }}>✓</span> : <span style={{ color: "#C7C7CC" }}>—</span>}
                      </td>
                      <td style={{ padding: "12px 14px", textAlign: "center", fontSize: "14px" }}>
                        {req.letterCopied ? <span style={{ color: "#34C759" }}>✓</span> : <span style={{ color: "#C7C7CC" }}>—</span>}
                      </td>
                      <td style={{ padding: "12px 14px", textAlign: "center" }}>
                        <span style={{ fontWeight: 900, fontSize: "15px", color: status.color }}>{score}</span>
                      </td>
                      <td style={{ padding: "12px 14px" }}>
                        <span style={{ background: status.bg, color: status.color, fontWeight: 700, fontSize: "11px", padding: "3px 9px", borderRadius: "9999px", border: `1px solid ${status.color}30`, whiteSpace: "nowrap" }}>
                          {status.label}
                        </span>
                      </td>
