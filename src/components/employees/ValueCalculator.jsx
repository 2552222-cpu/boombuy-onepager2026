...
// Custom ILS wrapper that formats numbers with he-IL locale
function FormattedILS({ value, ...props }) {
  const num = Math.round(Number(value ?? 0));
  const formatted = num.toLocaleString("he-IL");
  return <ILS value={formatted} {...props} />;
}
...
function LiveTotal({ answers }) {
  ...
  return (
    ...
    >
      ...
      <FormattedILS value={monthly} style={{ fontSize: 15, fontWeight: 800, color: "#0055CC" }} />
            ...
            {stage === "result" && (
              <motion.div key="result" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.4 }}>
                ...
                <div style={{ textAlign: "center", marginBottom: 4 }}>
                  <FormattedILS value={result.monthly} style={{ fontSize: "clamp(52px, 12vw, 80px)", fontWeight: 900, color: "#0055CC", letterSpacing: "-0.04em", lineHeight: 1 }} />
                </div>
                ...
                <p style={{ textAlign: "center", fontSize: 16, fontWeight: 700, color: "#1D1D1F", marginBottom: 28 }}>
                  וזה בערך <FormattedILS value={result.yearly} style={{ fontSize: 16, fontWeight: 900, color: "#0055CC" }} /> בשנה
                </p>
                  ...
                  <div style={{ background: "#F5F5F7", borderRadius: 18, padding: "18px 20px", marginBottom: 24, display: "flex", flexDirection: "column", gap: 10 }}>
                    {result.breakdown.map((b) => (
                      <div key={b.label} style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                        ...
                        <FormattedILS value={b.monthly} style={{ fontSize: 14, fontWeight: 800, color: "#1D1D1F" }} />
                      </div>
                    ))}
                  </div>
                )}
